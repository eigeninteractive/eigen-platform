...
Change any of it, since it is your repository, but the generated files are written
...
so it would fail on every push until both exist. Add it when you are ready to
...
calls first, and it needs no secrets, since `app-config.json`,
