...
`GameSummary` carries `seq`, the game's revision, the same counter a `Session` carries. Every D1 mirror write now names the commit it mirrors and lands only when the row is not already newer, so a retried older write no longer overwrites a newer summary or roster. D1 gains `games.seq` and a `game_finishes` table whose AUTOINCREMENT `seq` is the history sync cursor, and drops the rating-history index nothing reads any more. These fold into migration `0001`, which is rewritten rather than added to.

Migrating: `0001` is no longer the migration a database may already have applied under that tag, so recreate the database rather than migrating onto it. Pre-1.0, the schema is rewritten in place rather than accumulating a step per change. A client that listed games, ratings, or friends through the removed routes reads them from `syncAccount` instead.
