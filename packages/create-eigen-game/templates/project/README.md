...
`.github/workflows/release.yml` builds a signed, obfuscated Android App
Bundle on every push to `main` and uploads it to the Play Store internal
track via `app/fastlane`. `.github/workflows/checks.yml` is the PR gate it
...
| `android/app/src/main/res/drawable/ic_notification.xml` | Monochrome notification silhouette — a `<vector>`, not an SVG, and not generated by any tool |
