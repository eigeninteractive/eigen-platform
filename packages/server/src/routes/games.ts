...
import { type GameWithRoster, isAcceptedFriend, readBots, readGame, readGameByCode } from "../d1/reads.js";
...
import { actionBody, addBotBody, commandAcceptedShape, createdShape, createGameBody, errorShape, frameShape, joinBody, joinByCodeBody, lobbyAcceptedShape, lobbyCommandBody } from "./wire.js";
...
export function registerGameRoutes(app: EngineApp, ctx: RouteContext): void {
  ...
  // §4.2 add-bot — worker policy: guest rejection, registry gates
  // (schema, rated eligibility, server-only, timed invariant), botSeatable.
  app.openapi(
    createRoute({
      method: "post",
    ...
    async (c) => {
      ...
      // This route seats server bots exclusively: a local bot is driven by
      // the sole human's client and is seated at creation (solo games —
      // a later milestone), never into a multiplayer waiting room.
      if (bot.isLocal) throw new HttpError(400, "Local bots cannot join multiplayer games");
      // A server bot's endpoint may be unreachable; the turn deadline is the
      // designed liveness backstop for a lost wake — so the game must be
      // timed (§7).
  ...
  // §4.3 — a player's move. No seat and no D1 read: the DO resolves the
  // acting seat against its own roster (the authoritative copy — the D1
  // participants mirror only displays) and the caller's committed frame
  // rides the response.
  app.openapi(
    createRoute({
      method: "post",
  ...
  app.openapi(
    ...
    async (c) => {
      ...
      const cmd: Command = {
        kind: "lifecycle",
        gameId,
        commandId: c.req.valid("json").command_id ?? crypto.randomUUID(),
        actor: { userId: c.var.auth.user.id, botId: null },
        type: "forfeit",
      };
      return c.json(commandResult(await ctx.stub(c.env, gameId).handle(cmd)), 200);
