...
Then subset, convert and encode:
...
node -e 'const b=require("fs").readFileSync("inter.woff2").toString("base64");
  require("fs").writeFileSync("inter.ts", `export default "${b}";\n`)'
```

Keep the header comment, and bump `VERSION` in `../fonts.ts` — the URLs are
served `immutable`, so a returning visitor keeps the old bytes for a year
otherwise.
