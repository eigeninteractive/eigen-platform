...
describe("middleware + provisioning", () => {
  it("401s without a token and with garbage", async () => {
    expect((await SELF.fetch("https://x/api/engine/me")).status).toBe(401);
    expect((await SELF.fetch("https://x/api/engine/me", { headers: { authorization: "Bearer nope" } })).status).toBe(401);
  });
  ...
  it("provisions a users row on first sight, idempotently, deriving the handle from the email", async () => {
    ...
    const first = await SELF.fetch("https://x/api/engine/me", { headers });
    ...
    const again = (await (await SELF.fetch("https://x/api/engine/me", { headers })).json()) as { username: string };
    expect(again.username).toBe(profile.username);
  });

  it("suffixes the handle on collision; guests get a generated one", async () => {
    const one = (await (await SELF.fetch("https://x/api/engine/me", { headers: await bearer({ uid: freshUid(), email: "alice@a.com" }) })).json()) as { username: string };
    expect(one.username).toBe("alice");
    // Same local part, different provider account: retry appends 4 digits.
    const two = (await (await SELF.fetch("https://x/api/engine/me", { headers: await bearer({ uid: freshUid(), email: "alice@b.com" }) })).json()) as { username: string };
    expect(two.username).toMatch(/^alice_\d{4}$/);

    const guest = (await (await SELF.fetch("https://x/api/engine/me", { headers: await bearer({ uid: freshUid(), anonymous: true }) })).json()) as { username: string };
  ...
  it("backfills on guest → permanent conversion, preserving uid and handle; the provider identity overwrites", async () => {
    const uid = freshUid();
    const guest = (await (await SELF.fetch("https://x/api/engine/me", { headers: await bearer({ uid, anonymous: true }) })).json()) as { username: string; is_anonymous: boolean; email: string | null };
