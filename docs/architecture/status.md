...
| Commerce clients | Generated OpenAPI/Dart models, the pure-Dart `CommerceRepository`, the Flutter `PurchaseGateway`/`HostedStorefront` ports with `CommerceService`, and shell-safe creation retries are implemented. `eigen_commerce_play` implements Play Billing and `eigen_commerce_hosted` implements Stripe and Razorpay hosted checkout; neither has been run against a provider sandbox. Storefront presentation remains application work. |
