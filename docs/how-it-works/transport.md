...
re-projects from its immutable log), so replay is just the whole range rather than
...
is dropped. That matters less for latency than it looks, since the socket
terminates at the same Durable Object and is written first, but it is what makes the
